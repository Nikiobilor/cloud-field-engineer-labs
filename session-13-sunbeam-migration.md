...
## 13. Portfolio deliverable — Architecture Decision Record

**Note on what this documents:** this ADR describes the real engagement outcome — in an actual client environment, Corvex's physical MicroCeph cluster would never be destroyed and rebuilt, only the control plane migrates around it. That's different from what you just did in this lab, where MicroCeph was rebuilt from scratch for reproducibility (Section 12). The ADR reflects the correct real-world decision either way; it's a good exercise in writing the deliverable for the engagement you're simulating, not for the lab mechanics underneath it.

```markdown
# ADR-013: Migrate Corvex Private Cloud Control Plane from MicroStack to Sunbeam

## Status
Accepted — implemented 2026-07-11

## Context
Corvex Manufacturing's private cloud proof-of-concept was built on MicroStack,
which Canonical has officially discontinued. This leaves the control plane
without upstream security patching, which fails Corvex's Q3 infrastructure
compliance attestation.

## Decision
Migrate the OpenStack control plane to Sunbeam (Canonical's currently
supported `openstack` snap distribution), while preserving the existing
MicroCeph storage cluster and its Cinder-backed volumes unchanged.

## Consequences
- Positive: control plane is back on a supported, patched distribution;
  operational model now uses Juju, which is the same tool Corvex's
  eventual multi-node production cloud will be managed with — this
  migration is also a rehearsal for that later, larger change.
- Positive: no data migration was required — MicroCeph and its RBD pool
  were reconnected, not rebuilt, so existing volumes were preserved with
  zero data movement.
- Negative: Sunbeam's Juju-based model has a steeper operational learning
  curve than MicroStack's simpler single-binary approach — documented in
  the accompanying runbook for Corvex's internal team.
- Risk carried forward: this remains a single-node deployment. It resolves
  the compliance/support-status risk but not the availability risk of
  single-node storage or single-node control plane. Recommend Corvex scope
  a follow-on engagement for MAAS-managed multi-node deployment.

## Alternatives considered
- Remain on MicroStack, unsupported: rejected — fails compliance outright.
- Migrate to a different OpenStack distribution (e.g. Kolla-Ansible):
  rejected — Corvex's existing Canonical support contract makes Sunbeam
  the lower-total-cost option, and keeps them aligned with Canonical's
  own tooling direction for any future MAAS/bare-metal expansion.
```
...
```bash
# One-line explanation: removes the Sunbeam OpenStack model and Juju controller
# cleanly before destroying the underlying host, so nothing is left orphaned
# in Juju's own bookkeeping.
sunbeam cluster list
juju destroy-controller sunbeam-controller --destroy-all-models --no-prompt

# One-line explanation: destroys the Terraform-managed EC2 instance, which
# takes MicroCeph down with it since it lived on the same host — there's
# nothing to preserve between sessions, by design. Session 14 provisions its
# own fresh host and rebuilds MicroCeph again from there, multi-node this time.
terraform destroy -var="allowed_ssh_cidr=$(curl -s ifconfig.me)/32" -var="key_pair_name=linux"
```
...
The sharp moment worth carrying out of this session, if you're looking for LinkedIn material later: the *decision* being made in this migration was that the storage layer stays put and only the orchestration layer changes — even though, mechanically, this lab rebuilt both from scratch for reproducibility. Knowing which layer of a stack actually holds the risk, and which layer is safely replaceable, is the kind of judgment a client is paying for. The `juju relate` syntax isn't the point; recognizing that Ceph is the thing you protect and Sunbeam is the thing you're allowed to redeploy is.
