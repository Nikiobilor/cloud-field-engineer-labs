...
## Step 4 — Reproducing the Broken State with a Network Namespace
...
This is the error the RetailEdge team was seeing. The kernel checked the routing table, found no matching entry for 8.8.8.8, and rejected the packet locally — it never even left the host.
...
sudo ip netns exec private-sim ip route add default via 10.99.0.1
...
The second root cause in the RetailEdge ticket was that the app-tier security group used a CIDR block as the source rule for port 8080. This section explains exactly why that is a problem, when you would use each approach, and how to write the correct configuration.
...
This says: allow port 8080 from any IP address in `10.0.1.0/24`. On day one, when there is one EC2 instance in that subnet, this works. But:
...
# the module. The root module references outputs to place the EC2 instance.
...
- `aws_vpc.main` — the VPC itself
- `aws_internet_gateway.main` — the IGW
...
```bash
# Describe route tables tagged for this environment
...
The private subnet currently has no outbound internet route. For this session's learning objective — demonstrating subnet isolation — that is intentional. But in a real environment, the backend service needs to pull package updates, reach AWS services (S3, Secrets Manager), and send logs to an external sink. That requires a NAT Gateway.
...
Every piece of infrastructure you build for a client must be documented in terms they can act on — not just a list of what was created, but why each decision was made. A routing decision log is the architectural equivalent of a commit message: it records the reasoning so the next engineer does not have to reverse-engineer it from the resource configuration.
...
Route Table (private)               rtb-0xxx...             local route only
...
6. HOW TO VERIFY
  ...
  aws ec2 describe-route-tables ...         → confirm IGW route on public RT
...
- NAT Gateway pattern written out in commented Terraform — uncomment to deploy
...
| Private namespace can reach internet unexpectedly | `sudo ip netns exec private-sim ip route show` | A default route was added during the fix demo; remove with `sudo ip netns exec private-sim ip route del default` |
...
| Internet Gateway vs NAT Gateway — when and why | Clients routinely conflate these. Being able to explain the difference, the cost model, and the security implication of each places you above most engineers who just follow a tutorial. |
...
A NAT Gateway charges per GB processed in addition to the hourly fee. For a workload that primarily communicates with S3, SSM, ECR, and Secrets Manager, all outbound traffic to those services travels through the NAT Gateway and incurs data transfer charges. A Gateway Endpoint for S3 is free and routes S3 traffic inside the VPC, bypassing the NAT Gateway entirely. Interface Endpoints (PrivateLink) for other AWS services cost ~$7/month per AZ per endpoint. At what data volume does replacing a NAT Gateway with VPC Endpoints break even? When would you recommend keeping the NAT Gateway regardless?

**4. CloudTrail vs VPC Flow Logs — business context:**
A client's security team asks: "We need an audit trail that shows who made infrastructure changes and a separate log showing what network traffic occurred." Which tool answers the first question and which answers the second? CloudTrail records API calls to the AWS control plane — who called `RunInstances`, who modified a security group, who deleted a route. VPC Flow Logs records data plane traffic — which IP connected to which port, and whether it was accepted or rejected. Can either tool replace the other? In a compliance conversation (PCI-DSS, ISO 27001), which would you enable first, and why? How does this distinction map to the difference between auditd (which you configured in Session 7) and nginx access logs — which layer does each operate at?
...
**Session 9:** IAM roles and instance profiles — giving the EC2 web tier permission to call AWS services (S3, Secrets Manager) without storing static credentials, and auditing what permissions are actually being used with IAM Access Analyzer.
